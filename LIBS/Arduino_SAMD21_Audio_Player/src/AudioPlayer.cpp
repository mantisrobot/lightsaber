...
{                                      // Interrupt Service Routine for timer TC4
  ...
  if (sampleInterruptCounter == 0) 
  {                      // If this is the first pass for this sample:
  ...
  if (sampleInterruptCounter >= overSampling) 
  {           // When interpolation has been handled:
    ...
    if (sampleNumber >= sampleSize) 
		{                     // At the end of the samples array:
			if( !loopEnable )
			{
				REG_TC4_CTRLA &= ~TC_CTRLA_ENABLE;                  // Disable timer TC4
				while (TC4->COUNT16.STATUS.bit.SYNCBUSY);           // Wait for synchronization
...
int samplePlaying()
{
	return( REG_TC4_CTRLA & TC_CTRLA_ENABLE  );
}